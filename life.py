...
def parse_rule(rule):
    ...
    birth_set = {int(n) for n in birth[1:]}       # skip 'B'
    survival_set = {int(n) for n in survive[1:]}  # skip 'S'
    return (birth_set, survival_set)

def parse_state(state):
    live_cells = set()
    rows = len(state)
    cols = max((len(row) for row in state), default=0)
...
def evolve_state(live_cells, rule):
    ...
    neighbor_counts = {}
    next_cells = set()
    # Include live cells even if they end up with 0 neighbors,
    # so survival rules like S0 can be applied
